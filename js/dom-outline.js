...
var DomOutline = function (options) {
    ...
    function updateOutlinePosition(e) {
        ...
        var win_width = $(window).width() - b;
        var win_height = $(window).height() - b;

        self.elements.label.css({ top: label_top, left: label_left }).text(label_text);
        
        self.elements.top.css({ top: Math.max(0, top - b), left: pos.left - b, width: Math.min(pos.width + b, win_width), height: b });
        self.elements.bottom.css({ top: top + pos.height, left: pos.left - b, width: Math.min(pos.width + b, win_width), height: b });
        self.elements.left.css({ top: top - b, left: Math.max(0, pos.left - b), width: b, height: Math.min(pos.height + b, win_height) });
        self.elements.right.css({ top: top - b, left: pos.left + pos.width, width: b, height: Math.min(pos.height + b*2, win_height) });
    }
